...
import { GlobalExceptionFilter, ResponseInterceptor, RequestIdMiddleware } from '@chat-app/common';
...
async function bootstrap(): Promise<void> {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('Stateless HTTP Gateway and REST boundaries for RelayFlow realtime platform')
    .setVersion('1.0')
    .addBearerAuth({
      type: 'http',
      scheme: 'bearer',
      bearerFormat: 'JWT',
      description: 'Input your Bearer JWT token to access authenticated REST routes',
    })
    .build();
