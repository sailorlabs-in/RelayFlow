  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Body */}
        <form onSubmit={handleSubmit} className="px-5 py-5 max-h-[70vh] overflow-y-auto">
          <div className="mb-4">
            <label
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-3">
              <label className={`flex-1 flex flex-col p-3 rounded-[10px] border-[1.5px] cursor-pointer transition-all ${layout === 'text' ? 'border-[var(--accent-primary)] bg-[rgba(114,137,218,0.1)]' : 'border-[var(--glass-border)] bg-transparent'}`}>
                <input
                  type="radio"
                  name="channelLayout"
                  value="text"
                  ...
                  className="hidden"
                />
                <span className="text-sm font-semibold text-[var(--text-primary)]">Text Channel</span>
                <span className="text-xs text-[var(--text-muted)] mt-1">Classic flat feed</span>
              </label>
              <label className={`flex-1 flex flex-col p-3 rounded-[10px] border-[1.5px] cursor-pointer transition-all ${layout === 'bubble' ? 'border-[var(--accent-primary)] bg-[rgba(114,137,218,0.1)]' : 'border-[var(--glass-border)] bg-transparent'}`}>
                <input
                  type="radio"
                  name="channelLayout"
                  value="bubble"
                  ...
                  className="hidden"
                />
                <span className="text-sm font-semibold text-[var(--text-primary)]">Conversation Channel</span>
                <span className="text-xs text-[var(--text-muted)] mt-1">WhatsApp-style bubble feed</span>
              </label>
            </div>
          </div>
          ...
          <div className="mb-4 mt-5 p-3 rounded-[10px] border-[1.5px] border-[var(--glass-border)] bg-[rgba(255,255,255,0.02)]">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm font-semibold text-[var(--text-primary)]">Private Channel</span>
              ...
              <div className="mt-4 border-t border-[var(--border-muted)] pt-3">
                ...
                {roles.length === 0 ? (
                  <p className="m-0 text-xs text-[var(--text-muted)] italic">
                    No custom roles exist. Create roles in Server Settings first, or only Owners/Admins will access this channel.
                  </p>
                ) : (
