      ...
      {mediaItems.map((item, idx) => {
        ...
        if (isImage) {
          ...
          return (
            <div
              key={idx}
              className="mt-1 max-w-full w-fit rounded-lg overflow-hidden border border-theme bg-theme-input hover:scale-[1.01] transition-all duration-200 h-[360px] min-w-[240px]"
            >
              <img
                src={displayUrl}
                alt={item.name || 'Image'}
                className="h-full w-auto max-w-full min-w-[240px] object-cover cursor-pointer hover:opacity-90 transition-opacity"
        ...
        if (isVideo) {
          ...
          return (
            <div
              key={idx}
              className="mt-1 max-w-full w-fit rounded-lg overflow-hidden border border-theme bg-theme-input relative cursor-pointer hover:scale-[1.01] transition-all duration-200 group h-[360px] min-w-[240px]"
              onClick={() => onMediaClick(item)}
            >
              {item.thumbnailUrl ? (
                <div className="relative h-full w-fit">
                  <img
                    src={displayUrl}
                    alt={item.name || 'Video'}
                    className="h-full w-auto max-w-full min-w-[240px] object-cover transition-opacity"
                  />
                ...
                <video
                  src={item.url}
                  className="h-full w-auto max-w-full min-w-[240px] object-cover"
                  preload="metadata"
                />
              )}
            </div>
