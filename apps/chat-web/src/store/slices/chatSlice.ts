...
export const createConversation = createAsyncThunk(
  ...
  async (payload: { userIds: string[]; recipient: User }, { getState, rejectWithValue }) => {
    try {
      const state = getState() as { auth: { accessToken: string | null } };
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    socketReceiveMessage: (state, action: PayloadAction<Message>) => {
      ...
      const convoIndex = state.conversations.findIndex((c) => c.id === msg.conversationId);
      if (convoIndex !== -1) {
      ...
      .addCase(fetchConversations.fulfilled, (state, action) => {
        ...
        action.payload.forEach((convo: any) => {
          if (convo.type === 'dm' && convo.members) {
            const recipient = convo.members.find((m: any) => m.userId !== currentUserId);
            if (recipient) {
